...
public class SC006_Payment_API extends BaseClass {
	ExcelUtility x = new ExcelUtility("SC006");
	String courseName;

	@Test(enabled = true, priority = 0)
	public void SC006_Payment_API() throws Exception {
		...
		password = x.readData("Password");
		email = objSignUp.signUpForNewUser(x.readData("FirstName"), x.readData("LastName"), x.readData("Country"), mobileNumber,
				email, password);	
		...
		objPaymentPage.enterPaymentScreenDetailsAndPay(x.readData("DOB"),x.readData("Qualification"),x.readData("QualificationName"));
