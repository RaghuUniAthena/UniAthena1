...
public class SC010_Payment_ManualSettlement_EmailId extends BaseClass {
	ExcelUtility x = new ExcelUtility("SC006");
	String courseName;

	@Test(enabled = true, priority = 0)
	public void SC010_Payment_ManualSettlement_EmailId() throws Exception {
...
//		String orderId = objPaymentPage.fetchOrderId();
		...
		objAdmin_Settlement.searchByEmailId(email);
		objAdmin_Settlement.applySettlementByEmailId(email);
		...
		objAdmin_SettledPayments.searchByEmailId(email);
		objAdmin_SettledPayments.verifyIfEmailIdIsSettled(email, true);

		u.launchUrl(adminDashboard);
		objAdmin_HomePage.clickSettlementCard();
		objAdmin_Settlement.searchByEmailId(email);
		objAdmin_Settlement.verifyIfEmailIsSettled(email, false);
